...
func DecisionCheckpoint(
	systemInfoSet []global.SystemInfo,
	resultChan1 chan global.CheckpointContainer,
	resultChan2 chan global.CheckpointContainer,
	...
	if global.IS_STATEFUL {
		for _, pauseContainer := range pauseContainerList {
			...
			if isAppend {
				...
				container.PodUid = pauseContainer.PodUid
				container.ContainerData = pauseContainer.ContainerData
				container.ScaleSize = pauseContainer.ScaleSize
	...
	for _, checkpointContainer := range updateCandidateList {
		...
		go func(container global.CheckpointContainer) {
			...
			if GetContainerFreeze(checkpointContainer.PodUid) == "1" {
				ContinueContainer(checkpointContainer.PodUid, checkpointContainer.ContainerData)
			}
