	...
	if float64(memory.Used)+float64(scaleUpMemorySize) < float64(memory.Total)*global.MAX_MEMORY_USAGE_THRESHOLD {
		...
		for _, scaleCandidate := range scaleUpCandidateList {
			...
			go func(scaleCandidate global.ScaleCandidateContainer) {
				...
				if GetContainerFreeze(scaleCandidate.PodUid) == "1" {
					ContinueContainer(scaleCandidate.PodUid, scaleCandidate.ContainerData)
				}
				ScaleUp(client, scaleCandidate.ContainerData, scaleCandidate.ScaleSize)
				// update container info
				UpdateContainerData(client, scaleCandidate.ContainerData)
				global.NumOfTotalScale++
			...
			go func(scaleCandidate global.ScaleCandidateContainer, i int) {
				...
				if lastScaleUpSize != 0 && i == len(targetScaleUpList)-1 {
					...
					if GetContainerFreeze(scaleCandidate.PodUid) == "1" {
						ContinueContainer(scaleCandidate.PodUid, scaleCandidate.ContainerData)
						// update container info
						// update를 하려면 continue가 발생했을 때 freeze된 시간 체크해서 줄이는 쪽 빼곤 없을거 같다
						//UpdateContainerData(client, scaleCandidate.ContainerData)
					}
					ScaleUp(client, scaleCandidate.ContainerData, scaleCandidate.ScaleSize)
				} else {
		...
		// Sort scaleUpCandidateList with lowest priority
		for i, j := 0, len(scaleUpCandidateList)-1; i < j; i, j = i+1, j-1 {
			scaleUpCandidateList[i], scaleUpCandidateList[j] = scaleUpCandidateList[j], scaleUpCandidateList[i]
		}
		...
		for _, pauseCandidate := range scaleUpCandidateList {
			go func(pauseCandidate global.ScaleCandidateContainer) {
				defer wg.Done()
				// Check for pauses
				if CheckToPauseContainer(pauseCandidate, pauseContainerList) && GetContainerFreeze(pauseCandidate.PodUid) == "0" { // if not pause yet
					PauseContainer(pauseCandidate.PodUid, pauseCandidate.ContainerData)
	...
	for i := 0; i < len(pauseContainerList); i++ {
		...
		if isRequireContinue {
			...
			go func(pauseContainerList []global.PauseContainer, i int) {
				...
				if GetContainerFreeze(pauseContainerList[i].PodUid) == "1" {
					ContinueContainer(pauseContainerList[i].PodUid, pauseContainerList[i].ContainerData)
				}
				pauseContainerList[i].ContainerData.PauseTime += time.Now().Unix() - pauseContainerList[i].Timestamp

				// update container info
				UpdateContainerData(client, pauseContainerList[i].ContainerData)
		...
		for i, container := range pod.Container {
			...
			if conMemUtil > global.CONTAINER_MEMORY_SLO_UPPER {
				...
				if CheckToAppendScaleCandidateList(podName, container, scaleUpCandidateList) {
					...
					scaleUpCandiate.PodUid = pod.Uid
...
func AppendToScaleDownCandidateList(client internalapi.RuntimeService, scaleDownCandidateList []global.ScaleCandidateContainer, podIndex map[string]int64, podInfoSet []global.PodData, currentRunningPods []string) ([]global.PodData, []global.ScaleCandidateContainer) {
	...
	for _, podName := range currentRunningPods {
		...
		for i, container := range pod.Container {
			...
			if time.Now().Unix()-(pod.Container[i].StartedAt/global.NANOSECONDS) < global.START_THRESHOLD {
				continue
			}
			...
			if int64(float64(res[len(res)-1].MemoryUsageBytes)) < global.MIN_SIZE_PER_CONTAINER {
				...
				if container.Cgroup.MemoryLimitInBytes >= global.MIN_SIZE_PER_CONTAINER {
					if float64(res[len(res)-1].MemoryUsageBytes)/float64(global.MIN_SIZE_PER_CONTAINER) <= global.CONTAINER_MEMORY_SLO {
						ScaleDown(client, &container, global.MIN_SIZE_PER_CONTAINER)
						// update container info
						UpdateContainerData(client, &pod.Container[i])

						// reset TimeWindow
						pod.Container[i].TimeWindow = 1
						// reset container resource slice
						pod.Container[i].Resource = container.Resource[len(container.Resource)-1:]
						continue
					}
				}
			}
...
func ScaleUp(client internalapi.RuntimeService, scaleUpCandidate *global.ContainerData, scaleUpSize int64) {
	...
	fmt.Println("ScaleUp")
	UpdateContainerResources(client, scaleUpCandidate.Id, scaleUpCandidate.OriginalContainerData)
}
...
func ScaleDown(client internalapi.RuntimeService, scaleDownCandidate *global.ContainerData, scaleDownSize int64) {
	...
	UpdateContainerResources(client, scaleDownCandidate.Id, scaleDownCandidate.OriginalContainerData)
}
